...
function header(sessionId: string, transactionId = sessionId): string {
  return `<SOAP-ENV:Header><cai3g:SequenceId>${escapeXml(sessionId)}</cai3g:SequenceId><cai3g:TransactionId>${escapeXml(transactionId)}</cai3g:TransactionId><cai3g:SessionId>${escapeXml(sessionId)}</cai3g:SessionId></SOAP-ENV:Header>`;
}
...
function envelope(
  ...
  return `<SOAP-ENV:Envelope xmlns:SOAP-ENV="${SOAP}" xmlns:cai3g="${CAI}" xmlns:gsm="${HLR}">${header(sessionId, transactionId)}<SOAP-ENV:Body>${body}</SOAP-ENV:Body></SOAP-ENV:Envelope>`;
}
...
export function deleteHlr(sessionId: string, msisdn: string): string {
  return envelope(
...
export function unbarInternet(sessionId: string, msisdn: string): string {
  return envelope(
